...
if ARGV.size < 4
    ...
    puts "Usage: ./run_one.rb numFlows bufSize runTimeInSec synchSize"
    exit
end
...
system("./gen_ns_file.rb #{flows} #{synchSize} #{bufSizeInPackets} #{runTimeInSec} #{runType}")
...
num_avg = `cat #{TEMPDIR}/tout.dat | grep " #{synchSize} " | #{BASEDIR}/column.pl -t " " 2 | ../../../tools/UnixStat/bin/stats mean`
...
num_avg = `cat #{TEMPDIR}/rexmit.dat | grep " #{synchSize} " | #{BASEDIR}/column.pl -t " " 2 | ../../../tools/UnixStat/bin/stats mean`
out.puts "#{bufSizeInPackets} #{flows} #{synchSize} #{num_avg}"
...
out.puts "set terminal postscript eps monochrome;"
out.puts "set size 0.65"
out.puts "set output '#{GRAPHDIR}/_graph_gput_#{runType}_flows_#{flows}_synch_#{synchSize}_buf_#{bufSizeInPackets}_time_#{runTimeInSec}_Magnified.eps';"
out.puts "set title \"Goodput, Timeout and Synchronization Events\";"
out.puts "set xlabel \"time (seconds)\"; set ylabel \"Mbps\";"
out.puts "set xrange [2:3]"
