...
for gtype in ["monochrome", "color"]
    ...
    out.puts "set output '#{GRAPHDIR}/_app_goodput_vs_servers_#{runType}_synch_#{synchSize/1000}K_#{gtype}.eps';"
...
=begin
    for DATA_DUMP_DIR in ["#{BASEDIR}/../../pdl/dell/256000"]
        out2 = File.new("../temp/gput_cum_avg_graph2_#{synchSize}_#{runNumber}.dat", "w")
        for flowId in 1..23
            puts "----- flow #{flowId} -----"
            prefixDir = "#{DATA_DUMP_DIR}"
