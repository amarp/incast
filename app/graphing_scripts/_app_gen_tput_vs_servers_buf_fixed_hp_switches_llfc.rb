...
for gtype in ["monochrome", "color"]
    ...
    out.puts "set output '#{GRAPHDIR}/_app_goodput_vs_servers_hp_llfc_#{runType}_synch_#{synchSize/1000}K_#{gtype}.eps';"
    ...
    runNumber = 1
    for DATA_DUMP_DIR in ["#{BASEDIR}/../../pdl/pdl.hp.llfc.1switch", "#{BASEDIR}/../../pdl/hp_noQOS_noLLFC"]
        ...
        run = ""
        if runNumber == 1
           #run = "HP Procurve 2848 (EFC enabled)" 
           run = "One switch, EFC enabled" 
        elsif runNumber == 2
           run = "One switch, EFC disabled" 
        end
